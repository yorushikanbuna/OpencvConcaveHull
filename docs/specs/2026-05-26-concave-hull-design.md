...
**执行流程**：
1. 检查命令行参数（必需：POS文件路径；可选：k值）
2. 运行 `RunTests()` 自测
3. `ParsePosFile(argv[1])` → 点集
4. 确定k值（自动确定；可接受 `argv[2]` 作为整数k值覆盖）
5. `ConcaveHull(点集, k)` → 凹包
6. `SaveHullToFile(凹包, ...)` → 文本输出
7. `Visualize(点集, 凹包, ...)` → 图像输出
